...
>
	{#if !product.disponible}
		<div class="absolute top-0 right-0 z-20 bg-red-500 text-white text-[9px] font-bold px-2 py-1 rounded-bl-xl shadow-sm">Agotado</div>
	{/if}
	...
	<div class="flex flex-col flex-1 gap-0.5">
		...
		<div class="flex items-start gap-0.5 mt-0.5 text-[#222] group-hover:text-[#f2421e] transition-colors duration-200">
			...
			<span class="font-sans font-bold text-2xl leading-none">{Number(product.preciopormayor).toFixed(2)}</span>
		</div>
		<div class="text-[12px] text-[#222] font-normal leading-tight font-sans mt-1">Cantidad mínima: {product.moq} unidades</div>
		<div class="relative h-5 overflow-hidden mt-1.5 w-full">
			{#key index}
				...
				>
					<span class="text-[11px] text-[#f2421e] font-medium leading-tight font-sans w-full truncate">{salesTags[index]}</span>
				</div>
			{/key}
		</div>
	</div>
