<script>
	...
	function comprarWhatsApp(e) {
		...
		// Mensaje 100% texto para evitar errores de codificación
...
{#if mostrarZoom}
	<div class="fixed inset-0 z-[9999] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm" on:click={cerrarZoom} role="button" tabindex="0">
		<div class="relative w-full max-w-lg bg-white rounded-xl overflow-hidden p-5 shadow-2xl animate-in fade-in zoom-in duration-200" on:click|stopPropagation role="dialog">
			...
			<div class="space-y-3">
				<h2 class="text-[16px] font-bold text-[#222] leading-tight uppercase">{product.name || product.descripcion}</h2>

				<div class="flex justify-between items-end border-b pb-2">
					<div>
						<span class="text-[10px] font-bold text-gray-400 uppercase">Precio Unidad x Mayor</span>
						<div class="text-[#ff4400] font-bold text-2xl tracking-tighter">Bs. {precioDisplay}</div>
					</div>
					<span class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded">Ref: {product.codigo || 'S/N'}</span>
				</div>

				{#if !estaAgotado}
					<div class="bg-orange-50 p-3 rounded-lg border border-orange-100 flex justify-between items-center">
						<span class="text-[11px] font-bold text-orange-800 uppercase">Mínimo: {minQty} Uds.</span>
						<div class="text-right">
							<span class="block text-[9px] text-orange-600 font-bold uppercase">Total Compra:</span>
							<span class="text-xl font-black text-[#ff4400]">Bs. {inversionTotal}</span>
						</div>
					</div>

					<button on:click={comprarWhatsApp} class="w-full flex items-center justify-center gap-3 bg-[#25D366] text-white py-3.5 rounded-xl font-bold text-[14px] uppercase shadow-lg active:scale-95 transition-transform">
...
{#if product}
	<div class="flex flex-col bg-white rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300 relative group h-full border border-gray-100 {estaAgotado ? 'opacity-70' : ''}">
		...
		<div class="relative w-full aspect-square bg-white overflow-hidden cursor-zoom-in" on:click={abrirZoom}>
