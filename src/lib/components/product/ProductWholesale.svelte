<script>
	...
	// Control de la Galería del Modal
	let activeImage;
	...
	function abrirZoom(e) {
		...
		activeImage = mainImage; // Al abrir, empieza con la principal
		mostrarZoom = true;
	}
			...
			<div class="bg-[#f8f8f8] rounded-lg overflow-hidden mb-2 relative">
				...
				{#if estaAgotado}
					<div class="absolute inset-0 flex items-center justify-center bg-white/50">
						<span class="bg-black text-white px-3 py-1 rounded-full font-bold text-xs">AGOTADO</span>
					</div>
				{/if}
			</div>
