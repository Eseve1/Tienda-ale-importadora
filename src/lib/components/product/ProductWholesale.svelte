...
{#if mostrarZoom}
	<div class="fixed inset-0 z-[9999] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm" on:click={cerrarZoom} role="button" tabindex="0" on:keydown={(e) => e.key === 'Escape' && cerrarZoom()}>
		<div class="relative w-full max-w-lg bg-white rounded-xl overflow-hidden p-2 shadow-2xl animate-in fade-in zoom-in duration-200" role="dialog">
			...
			<img src={mainImage} alt="" class="w-full h-auto object-contain max-h-[80vh] {estaAgotado ? 'grayscale' : ''}" />

			{#if estaAgotado}
				<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-red-600 text-white px-6 py-2 rounded-lg font-black text-xl uppercase tracking-widest shadow-xl border-2 border-white transform -rotate-12">
					AGOTADO
				</div>
			{/if}
...
{#if product}
	<a href="/product-{product.slug || product.$id}" class="flex flex-col bg-white border border-gray-100 rounded-[12px] overflow-hidden hover:border-orange-200 hover:shadow-lg transition-all duration-300 relative group no-underline h-full {estaAgotado ? 'opacity-70' : ''}">

		{#if esOfertaRelampago && !estaAgotado}
			<div class="absolute top-2 left-2 bg-[#FF0033] text-white text-[9px] font-bold px-2 py-0.5 rounded-[4px] z-20 shadow-sm animate-pulse">¡Oferta!</div>
		{/if}
      ...
      {hasSecondImage && !estaAgotado ? 'group-hover:opacity-0' : 'group-hover:scale-105'}
      {estaAgotado ? 'grayscale' : ''}"
					loading="lazy"
				/>

				{#if hasSecondImage && !estaAgotado}
					<img
						src={secondImage}
						alt="trasera"
              ...
              <span class="relative inline-flex rounded-full h-2 w-2 bg-orange-500"></span>
				...
				{#if estaAgotado}
					<div class="absolute inset-0 bg-white/40 flex items-center justify-center z-20">
						<span class="bg-gray-800 text-white px-2 py-1 rounded font-bold text-[10px] uppercase tracking-widest transform -rotate-6">Sin Stock</span>
					</div>
				{/if}
			{:else}
				<div class="absolute inset-0 flex items-center justify-center text-gray-200"><svg class="w-10 h-10" fill="currentColor" viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg></div>
			{/if}
		</div>

		<div class="p-3 flex flex-col flex-1">
			<h3 class="text-[13px] text-gray-700 leading-snug line-clamp-2 font-semibold hover:text-[#FF6600] mb-1 min-h-[2.5em] {estaAgotado ? 'text-gray-400' : ''}">
			...
			<div class="h-6 mb-1 relative overflow-hidden flex items-center">
				{#if !estaAgotado}
					{#key indexEtiqueta}
						<div class={`absolute top-0 left-0 text-[9px] font-bold px-2 py-0.5 rounded border ${etiquetas[indexEtiqueta].color} inline-block whitespace-nowrap shadow-sm`} transition:fade={{ duration: 300 }}>
				...
				<div class="flex items-end justify-between mb-1">
					<div class="flex flex-col">
						...
						<div class="flex items-baseline gap-0.5">
							...
							<span class="text-[18px] font-bold leading-none {estaAgotado ? 'text-gray-400' : 'text-gray-900'}">{precioDisplay}</span>
						</div>
					</div>
					<div class="text-right flex flex-col items-end">
						<span class="text-[9px] text-gray-400">Ref: {product.codigo || 'S/N'}</span>
						{#if !estaAgotado}<span class="text-[9px] font-bold text-gray-500 bg-gray-50 px-1.5 rounded mt-0.5 border border-gray-100">Mín: {minQty} Uds.</span>{/if}
					</div>
				</div>
			</div>
			...
			{#if estaAgotado}
				<div class="w-full bg-gray-100 text-gray-400 border border-gray-200 rounded-lg py-1.5 text-center cursor-not-allowed font-bold text-[12px]">Sin Stock</div>
			{:else}
				<div class="w-full bg-white hover:bg-[#FF6600] text-[#FF6600] hover:text-white border border-[#FF6600] rounded-lg py-1.5 text-center cursor-pointer transition-colors active:scale-95 flex items-center justify-center gap-2 font-bold text-[12px]" on:click={comprarWhatsApp}>
