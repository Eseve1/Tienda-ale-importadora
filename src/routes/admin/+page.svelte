<script>
	...
	async function buscarProductos() {
		if(!searchTerm.trim()) { mostrarMensaje("Escribe algo para buscar", "error"); return; }
		loading = true; inventory = [];
		try {
			// Intento 1: Búsqueda nativa
			try {
				const res = await db.listDocuments(DATABASE_ID, COLLECTION_ID, [Query.search('descripcion', searchTerm), Query.limit(50)]);
				inventory = res.documents;
			} catch (e) {
				// Intento 2: Fallback manual (si no hay indices)
				const res = await db.listDocuments(DATABASE_ID, COLLECTION_ID, [Query.limit(100), Query.orderDesc('$createdAt')]);
				inventory = res.documents.filter(p => p.descripcion.toLowerCase().includes(searchTerm.toLowerCase()) || (p.codigo && p.codigo.toLowerCase().includes(searchTerm.toLowerCase())));
			}
			if(inventory.length === 0) mostrarMensaje("No se encontraron productos", "error");
		} catch (error) { console.error(error); mostrarMensaje("Error al buscar", "error"); }
		loading = false;
	}
	...
	async function guardarProducto() {
		loading = true;
		try {
			let url1 = form.imagen; let url2 = form.imagen2;
	...
	async function toggleStock(product) {
		const newState = !product.disponible; product.disponible = newState; inventory = [...inventory];
		try { await db.updateDocument(DATABASE_ID, COLLECTION_ID, product.$id, { disponible: newState }); }
		catch (e) { product.disponible = !newState; inventory = [...inventory]; alert("Error: No tienes permiso para editar."); }
	}

	function editar(product) {
		form = { ...product, id: product.$id }; preview1 = product.imagen; preview2 = product.imagen2;
	...
	function mostrarMensaje(texto, tipo) { msg = { text: texto, type }; setTimeout(() => msg = { text: "", type: "" }, 3000); }
</script>
			...
			<div class="lg:col-span-4 xl:col-span-3 lg:sticky lg:top-4 h-fit">
				<div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
					...
					<div class="space-y-4">
						...
						{#if msg.text}<div class="p-3 rounded text-center text-sm font-bold animate-in fade-in zoom-in {msg.type === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}">{msg.text}</div>{/if}
					</div>
				</div>
			</div>
				...
				<div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 min-h-[500px]">
					...
					{:else if inventory.length === 0}
						<div class="text-center py-20 text-gray-400 flex flex-col items-center">
							...
							<p class="font-bold text-gray-500">El inventario está oculto para mayor velocidad.</p>
							<p class="text-sm">Usa el buscador arriba.</p>
						</div>
					{:else}
